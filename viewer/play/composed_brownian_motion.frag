...
void program(inout vec3 color) {
    ...
    float n_time = (u_time + 20.) * .9;

    float movement = sin( u_time) * .5 + .2;


    float per1 = fbm(10. * st + n_time); // Coef is density
    // st = fluidWarp(3.*st);

    float z1 = perlin( 6. * st * per1); // Coeff is frequency
    float z2 = perlin( 5. * st * per1); // Coeff is frequency
    //z *= perlin(10. * st + u_time);

    float nb = .09;// sin(2.*u_time) * .5 + .2;

    float b1 = 1.-smoothstep(nb, nb + .2, z1);
    float b2 = 1.-smoothstep(nb, nb + .2, z2);
    ...
    #define background colorF
    #define primaryA   colorC
    #define primaryB   colorB


    vec3 phi = mix(primaryA, background, b1);

    phi = mix(primaryB, phi, b2);
