...
void program(inout vec3 color) {
    ...
    float n_time = (u_time + 20.) * .2;

    float movement = sin(n_time) * .2;


    float per1 = perlin(3. * st + n_time); // Coef is density

    float z1 = perlin( 6. * st * per1); // Coeff is frequency
    float z2 = perlin( 8. * st * per1); // Coeff is frequency
    ...
    #define background colorB
    #define primaryA   colorF
    #define primaryB   colorC
